...
#   ax2.set_ylim(0, 1.1)
  ...
  for j in range(3):
    out = abs(store_arr[0,j] - store_arr[365,j]) / APCP_storage
    # print(out)
    dS_arr[idx, j] = out
    del out
...
for l in range(3):
    ...
    if l == 0:
        ax[l].set_ylabel('perc change storage w respect to APCP')

fig.legend()
fig.savefig(f'{out_dir}00_all_deltastorage_log_APCP.png')
fig.savefig(f'{out_dir}00_all_deltastorage_log_APCP.eps', type='eps')
plt.show()
plt.close()
